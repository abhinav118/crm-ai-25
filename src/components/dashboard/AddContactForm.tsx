...
const AddContactForm: React.FC<AddContactFormProps> = ({ open, onOpenChange, onSubmit }) => {
  ...
  const { data: segments = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const segmentOptions = Array.from(new Set((data ?? []).map((seg) => seg.segment_name?.trim()).filter(Boolean)));
  ...
  const handleFormSubmit = async (data: ContactFormData) => {
    ...
    try {
      ...
      // Create new contact
      const { data: newContact, error } = await supabase.from("contacts").insert([contactData]).select().single();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Fill in the details to add a new contact to your database.</DialogDescription>
        </DialogHeader>
