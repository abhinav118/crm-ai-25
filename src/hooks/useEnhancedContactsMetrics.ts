...
interface ContactsSegment {
  segment_name: string;
  contacts_membership: any;
  updated_at: string;
}

// Helper function to safely parse contacts from JSONB
function safelyParseContacts(contactsData: any): Contact[] {
  if (!contactsData || !Array.isArray(contactsData)) {
    return [];
  }

  return contactsData.filter((contact: any) => {
    return (
      contact &&
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<EnhancedContactsMetrics> => {
      ...
      const inactiveContacts = allContacts?.filter(contact => 
        contact.status === 'inactive' || contact.status === 'opted_out'
      ) || [];
      ...
      if (segments && segments.length > 0) {
        for (const segment of segments) {
          // Safely parse contacts_membership
          ...
          const growthRate = previousPeriodContacts.length > 0 
            ? ((currentPeriodContacts.length - previousPeriodContacts.length) / previousPeriodContacts.length * 100)
            : currentPeriodContacts.length > 0 ? 100 : 0;

          // Calculate campaigns sent for this segment
          const segmentCampaigns = campaigns?.filter(campaign => 
            campaign.segment_name === segmentName
          ) || [];
          
          const campaignsSent = segmentCampaigns.reduce((sum, campaign) => 
            sum + (campaign.sent_count || 0), 0
          );
          ...
          const responseRate = campaignsSent > 0 
            ? (segmentResponses.length / campaignsSent * 100).toFixed(1)
            : '0.0';

          // Find last campaign date
          const lastCampaign = segmentCampaigns.length > 0 
            ? format(new Date(Math.max(...segmentCampaigns.map(c => new Date(c.created_at).getTime()))), 'MMM dd, yyyy')
            : 'No campaigns';

          // Calculate engagement rate
          const engagementRate = segmentContacts.length > 0 
            ? (segmentResponses.length / segmentContacts.length * 100).toFixed(0)
            : '0';
          ...
          const avgValue = campaignsSent > 0 && segmentResponses.length > 0
      ...
      const segmentData = segments?.map((segment, index) => {
        ...
        const engagementRate = segmentContacts.length > 0 
          ? Math.round((segmentLogs.length / segmentContacts.length) * 100)
          : 0;
